      ...
      getWeeks: function getWeeks(date) {
        ...
        for (var s = firstDayInWeek - 1; s > 0; s--) {
          if (this$1.showNotInMonthDays) {
            var notInMonthDate = sub_days(date, s);
            var dayNumber = format_1(notInMonthDate, 'D');
            var dayNumberFull = format_1(notInMonthDate, 'DD');

            week.push({
              dayNumber: dayNumber,
              dayNumberFull: dayNumberFull,
              fullDate: format_1(notInMonthDate, 'YYYY-MM-DD'),
              notInMonth: true,
            });
          } else {
        ...
        for (var d = 0; d < daysInMonth; d++) {
          ...
          if (week.length === 7) {
            ...
            for (var i = 0; i < 7 - weekLength; i++) {
              if (this$1.showNotInMonthDays) {
                var notInMonthDate$1 = add_days(currentDate, i + 1);
                var dayNumber$2 = format_1(notInMonthDate$1, 'D');
                var dayNumberFull$2 = format_1(notInMonthDate$1, 'DD');

                week.push({
                  dayNumber: dayNumber$2,
                  dayNumberFull: dayNumberFull$2,
                  fullDate: format_1(notInMonthDate$1, 'YYYY-MM-DD'),
                  notInMonth: true,
                });
              } else {
                week.push(weekDayNotInMonth);
